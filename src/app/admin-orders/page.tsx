...
function Page() {
    ...
    const [orderDetails, setOrderDetails] = useState<any>(null); // State for order details
    ...
    const handleUpdateOrderStatus = async (id: string) => { // New function to update order status
        const response = await fetch(`https://manager-rkz3.onrender.com/api/orders/${id}/status`, {
    ...
    const handleViewDetails = async (id: string) => { // New function to fetch order details
    ...
    const filteredOrders = orders.filter((order) => {
        ...
        const matchesDate = searchDate ? new Date(order.createdAt).toLocaleDateString() === new Date(searchDate).toLocaleDateString() : true;
        ...
        <div className="flex flex-col min-h-screen">
            ...
            <main className="flex-grow container mx-auto px-4 py-8">
                        ...
                        <tbody>
                            {filteredOrders.map((order) => (
                                <tr key={order._id}>
                                    ...
                                    <td className="border px-4 py-2">{order.totalPrice.toLocaleString("vi-VN")} ₫</td>
                                    <td className="border px-4 py-2">{order.status === "pending" ? "Chưa xác nhận" : "Đã xác nhận"}</td>
                ...
                >
                    {orderDetails && (
                        <div>
                            <p><strong>ID:</strong> {orderDetails._id}</p>
                            ...
                            <p><strong>Tổng giá:</strong> {orderDetails.totalPrice.toLocaleString("vi-VN")} ₫</p>
                            <h4>Sản phẩm:</h4>
                            <ul>
